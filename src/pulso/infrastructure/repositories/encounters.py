...
from pulso.domain.clinical_events import ClinicalEvent, Utterance
from pulso.domain.encounters import Encounter, EncounterState
from pulso.domain.orders import Order, OrderState, transition
from pulso.infrastructure.audit.ledger import append_audit_event, canonical_json
...
class EncounterRepository:
    ...
    def set_state(self, encounter_id: str, state: EncounterState, *, actor: str) -> Encounter:
        ...
        closed_at = utc_iso() if state == EncounterState.CLOSED else None
        ...
        with self.database.transaction() as connection:
            connection.execute(
                ...
                (state.value, int(critical), closed_at, encounter_id),
            )
            append_audit_event(
                connection,
                entity_type="encounter",
        ...
        with self.database.transaction() as connection:
            connection.execute(
                ...
                (id, encounter_id, event_id, category, destination, request, state,
                 clinician_id, signature, readback_text, idempotency_key, replaces_order_id,
