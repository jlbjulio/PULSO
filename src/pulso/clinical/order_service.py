...
from pulso.clinical.events import ORDER_TYPES, ClinicalEvent
from pulso.clinical.orders import Order, OrderState
from pulso.storage.repository import EncounterRepository
from pulso.storage.routing import destination_for
...
class OrderService:
    ...
    def begin(self, order_id: str, *, actor: str) -> Order:
        return self.repository.transition_order(order_id, OrderState.IN_PROGRESS, actor=actor)

    def complete(self, order_id: str, *, actor: str) -> Order:
        return self.repository.transition_order(order_id, OrderState.COMPLETED, actor=actor)
