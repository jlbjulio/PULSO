...
TERMINAL_STATES = {OrderState.COMPLETED, OrderState.CANCELLED, OrderState.REPLACED}
...
class Order(BaseModel):
    ...
    def signed_when_confirmed(self) -> Order:
        if self.state in {OrderState.CONFIRMED, OrderState.DISPATCHED} and not (
            self.clinician_id and self.signature
        ):
...
def replace(order: Order, replacement: Order) -> tuple[Order, Order]:
    if order.state in TERMINAL_STATES:
        raise ValueError("a terminal order cannot be replaced")
    old = order.model_copy(update={"state": OrderState.REPLACED, "updated_at": utc_now()})
    new = replacement.model_copy(update={"replaces_order_id": order.id})
    return old, new
